...
5. （选择）下列电路中，各用电器互不影响的是（\quad）\\
\quad A. 串联电路 \quad B. 并联电路 \quad C. 混联电路 \quad D. 断路\\
6. （填空）在串联电路中，电流\_\_\_\_\_；在并联电路中，各支路两端\_\_\_\_\_。\\
7. （判断）开关必须连接在电源和用电器之间，不能只接在用电器两端。（\quad）\\
8. 画电路图时，导线应画成\_\_\_\_\_、\_\_\_\_\_，电路呈\_\_\_\_\_形状。
\end{practice}
...
5. （填空）电流表必须\_\_\_\_\_联在电路中，电压表必须\_\_\_\_\_联在电路两端。\\
6. （选择）下列数据中最符合实际的是（\quad）\\
\quad A. 一节干电池电压为2V \quad B. 手机电池电压约3.7V \quad C. 人体安全电压为36V \quad D. 家用电冰箱电流约10A\\
7. （计算）通过某灯泡的电流为0.2A，5分钟内通过其横截面的电荷量是多少？\\
8. （判断）电压是电路中形成电流的原因，所以只要有电压就一定有电流。（\quad）
\end{practice}
...
5. （计算）某电阻两端加9V电压时，电流为0.3A。它的电阻多大？\\
6. （选择）下列关于电阻的说法正确的是（\quad）\\
\quad A. 导体中的电流越大电阻越小 \quad B. 导体两端电压越高电阻越大\\
\quad C. 电阻由导体本身决定 \quad D. 铜导线的电阻一定比铁导线小\\
7. （填空）$R = U/I$ 中，当$U=0$时$R$\_\_\_\_\_（等于0/不等于0），因为电阻是导体本身的\_\_\_\_\_。\\
8. （判断）超导体的电阻为零，电流通过时不会发热。（\quad）
\end{practice}
...
5. （计算）$R_1=2\ \Omega$和$R_2=3\ \Omega$串联在10V电源上。求总电阻、电流、$R_1$两端电压。\\
6. （填空）并联电路的总电阻\_\_\_\_\_（大于/小于）任何一个分电阻，因为并联相当于\_\_\_\_\_了导体横截面积。\\
7. （判断）并联电路中，电阻越大的支路分到的电流越小。（\quad）\\
8. 两个电阻$R_1=12\ \Omega$、$R_2=4\ \Omega$并联，总电阻是\_\_\_\_\_$\Omega$。
\end{practice}
...
5. （计算）一台"220V 1000W"的空调正常工作2小时，消耗多少度电？\\
6. （选择）下列用电器中，利用电流热效应工作的是（\quad）\\
\quad A. 电风扇 \quad B. 电视机 \quad C. 电饭煲 \quad D. 电冰箱\\
7. （填空）电功率公式$P = UI$，对于纯电阻还可以写为$P =$\_\_\_\_\_ = \_\_\_\_\_。\\
8. （判断）实际电压低于额定电压时，用电器的实际功率小于额定功率。（\quad）
\end{practice}
...
5. （填空）磁极间的相互作用：同名磁极互相\_\_\_\_\_，异名磁极互相\_\_\_\_\_。\\
6. （选择）关于磁感线，下列说法正确的是（\quad）\\
\quad A. 磁感线是真实存在的 \quad B. 磁感线在磁体外部从N极到S极\\
\quad C. 磁感线在磁体内部从N极到S极 \quad D. 磁感线不能相交，但可以弯曲\\
7. （判断）地磁场的N极在地理北极附近。（\quad）\\
8. 右手螺旋定则（安培定则）：用右手握住螺线管，四指指向\_\_\_\_\_方向，大拇指指向\_\_\_\_\_极。
\end{practice}
...
6. （计算）一台电动机两端电压为220V，通过的电流为5A，工作10分钟。消耗多少电能？\\
7. （选择）关于电磁感应现象，下列说法正确的是（\quad）\\
\quad A. 只要导体在磁场中运动就能产生感应电流 \quad B. 感应电流的方向与磁场方向和导体运动方向有关\\
\quad C. 电磁感应把电能转化为机械能 \quad D. 发电机利用通电导体在磁场中受力原理\\
8. （填空）电动机把\_\_\_\_\_能转化为\_\_\_\_\_能，其原理是\_\_\_\_\_。\\
9. （判断）换向器的作用是在线圈转过平衡位置时改变线圈中的电流方向。（\quad）\\
10. 同台电机既可以做电动机也可以做发电机——这种性质叫做电机的\_\_\_\_\_性。
\end{practice}
...
2. B\quad
3. 1.5，220\quad
4. ×（电流表内阻极小，直接接电源会短路烧毁）\quad
5. 串联，并联\quad
6. B\quad
7. $Q=It=0.2\times(5\times60)=60\ \text{C}$\quad
8. ×（还需要电路是闭合的）

\textbf{第三课 练一练：}
1. $I=U/R=3/10=0.3\ \text{A}$\quad
2. C\quad
3. 接入电路电阻丝的长度\quad
4. 小\quad
5. $R=U/I=9/0.3=30\ \Omega$\quad
6. C\quad
7. 不等于0，属性\quad
8. $\surd$

\textbf{第四课 练一练：}
1. $R_{\text{串}}=4+6=10\ \Omega$，$I=U/R=5/10=0.5\ \text{A}$\quad
2. 电压，支路\quad
3. ×（并联总电阻小于任一分电阻）\quad
4. 正\quad
5. $R=2+3=5\ \Omega$，$I=10/5=2\ \text{A}$，$U_1=IR_1=4\ \text{V}$\quad
6. 小于，增大\quad
7. $\surd$\quad
8. $\frac{1}{R}=\frac{1}{12}+\frac{1}{4}=\frac{1}{3}$，$R=3\ \Omega$

\textbf{第五课 练一练：}
1. $W=Pt=0.04\times10=0.4\ \text{kW·h}$（度）\quad
2. $U$，$I$；$I^2R$或$U^2/R$\quad
3. B\quad
4. $Q=I^2Rt=4\times5\times10=200\ \text{J}$\quad
5. $W=Pt=1\times2=2\ \text{kW·h}$（度）\quad
6. C\quad
7. $I^2R$，$U^2/R$\quad
8. $\surd$
...
\textbf{第七课 练一练：}
1. 磁场对电流的作用（通电导体在磁场中受力），电磁感应\quad
2. B\quad
3. ×（电能转化为机械能）\quad
4. 电生磁，磁生电\quad
5. 机械，电\quad
6. $W=UIt=220\times5\times600=6.6\times10^5\ \text{J}$\quad
7. B\quad
8. 电，机械，磁场对电流的作用\quad
9. $\surd$\quad
10. 可逆
