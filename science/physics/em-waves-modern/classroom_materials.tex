...
4. （计算）在距离点电荷$Q=4\times10^{-6}\ \text{C}$的$0.2\ \text{m}$处，电场强度多大？\\
5. （选择）关于电场线，下列说法正确的是（\quad）\\
\quad A. 电场线是闭合曲线 \quad B. 电场线从负电荷出发到正电荷 \quad C. 电场线越密电场越强 \quad D. 电场线可以相交\\
6. （填空）静电力与万有引力的公式形式\_\_\_\_\_（相似/不同），但静电力比万有引力\_\_\_\_\_得多。\\
7. （判断）电场强度是标量，只有大小没有方向。（\quad）
\end{practice}
...
4. （计算）在$U=500\ \text{V}$的两点间移动$q=2\times10^{-6}\ \text{C}$的电荷，电场力做功多少？\\
5. （填空）电势的单位是\_\_\_\_\_，电势差也叫\_\_\_\_\_。\\
6. （选择）关于电势能，下列说法正确的是（\quad）\\
\quad A. 正电荷在电势越高处电势能越大 \quad B. 负电荷在电势越高处电势能越大\\
\quad C. 电势能与电荷量无关 \quad D. 电场力做正功电势能增大\\
7. （判断）匀强电场中，沿电场方向距离越大，电势差越大。（\quad）
\end{practice}
...
4. （计算）平行板电容器电容$C=20\ \mu\text{F}$，两端电压$U=50\ \text{V}$。电容器带电量多大？\\
5. （填空）平行板电容器的电容与\_\_\_\_\_成正比，与\_\_\_\_\_成反比。\\
6. （选择）电容器的单位是（\quad）\\
\quad A. 伏特 \quad B. 安培 \quad C. 法拉 \quad D. 欧姆\\
7. （判断）电容器两极板间的距离越大，电容越大。（\quad）
\end{practice}
...
4. （计算）电源$E=6\ \text{V}$，$r=1\ \Omega$，外接$R=5\ \Omega$。求电流和路端电压。\\
5. （选择）当外电阻$R$增大时，路端电压$U$会（\quad）\\
\quad A. 增大 \quad B. 减小 \quad C. 不变 \quad D. 先增大后减小\\
6. （填空）电源的效率$\eta = \dfrac{U}{E}\times100\%$，$R$越大，效率越\_\_\_\_\_。\\
7. （判断）电源短路时，路端电压等于电动势。（\quad）
\end{practice}
...
4. （计算）一根$L=0.3\ \text{m}$导线通$I=2\ \text{A}$电流，与$B=0.5\ \text{T}$磁场成$30^\circ$角。安培力多大？\\
5. （填空）左手定则中，磁感线穿入\_\_\_\_\_心，四指指向\_\_\_\_\_方向，拇指指向\_\_\_\_\_方向。\\
6. （选择）关于安培力，下列说法正确的是（\quad）\\
\quad A. 电流与磁场平行时安培力最大 \quad B. 安培力方向一定垂直于磁场方向\\
\quad C. 安培力方向一定垂直于电流方向 \quad D. B和C都正确\\
7. （判断）磁感应强度$B$的方向就是小磁针N极的受力方向。（\quad）
\end{practice}
...
4. （计算）一个$\alpha$粒子（$q=3.2\times10^{-19}\ \text{C}$，$m=6.64\times10^{-27}\ \text{kg}$）以$v=2\times10^6\ \text{m/s}$垂直射入$B=1\ \text{T}$的磁场。轨道半径多大？\\
5. （填空）带电粒子垂直进入匀强磁场做\_\_\_\_\_运动。周期$T =$\_\_\_\_\_，与速度\_\_\_\_\_。\\
6. （选择）洛伦兹力的方向（\quad）\\
\quad A. 与速度方向相同 \quad B. 与磁场方向相同 \quad C. 与速度方向垂直 \quad D. 与电荷正负无关\\
7. （判断）洛伦兹力对运动电荷始终不做功。（\quad）
\end{practice}
...
4. （计算）一个$n=100$匝的线圈在$0.1\ \text{s}$内磁通量从$0.02\ \text{Wb}$均匀增加到$0.07\ \text{Wb}$。感应电动势多大？\\
5. （填空）$\Phi = BS$中，$S$是指\_\_\_\_\_的面积。磁通量的单位是\_\_\_\_\_。\\
6. （选择）关于楞次定律，下列说法正确的是（\quad）\\
\quad A. 感应电流的磁场总是与原磁场方向相反 \quad B. 感应电流的磁场总是阻碍原磁通量的变化\\
\quad C. 楞次定律和能量守恒没有关系 \quad D. 楞次定律只适用于导体切割磁感线\\
7. （判断）导体切割磁感线运动时，感应电动势的大小与速度成正比。（\quad）
\end{practice}
...
4. （计算）一理想变压器$n_1=1000$匝，$n_2=5000$匝，输入$U_1=220\ \text{V}$。输出电压$U_2$多大？\\
5. （选择）交流电的有效值是指（\quad）\\
\quad A. 交流电的平均值 \quad B. 与它热效应相等的直流电数值 \quad C. 交流电的峰值 \quad D. 交流电的瞬时值\\
6. （填空）我国交流电的频率为\_\_\_\_\_Hz，周期为\_\_\_\_\_s。家用220V电压的峰值为\_\_\_\_\_V。\\
7. （判断）变压器可以改变直流电的电压。（\quad）
\end{practice}
...
4. （计算）一列声波在水中$v=1500\ \text{m/s}$，频率$f=300\ \text{Hz}$。波长多大？\\
5. （填空）简谐运动中回复力$F =$\_\_\_\_\_。弹簧振子周期$T =$\_\_\_\_\_。\\
6. （选择）两列波相遇时，振动加强的区域和减弱的区域交替出现——这是波的（\quad）\\
\quad A. 反射 \quad B. 折射 \quad C. 干涉 \quad D. 衍射\\
7. （判断）单摆的周期与摆幅大小无关（小角度范围内）。（\quad）
\end{practice}
...
5. （计算）紫光频率$\nu=7.5\times10^{14}\ \text{Hz}$，求一个光子能量。（$h=6.63\times10^{-34}\ \text{J·s}$）\\
6. （选择）关于原子结构，卢瑟福的$\alpha$粒子散射实验证明（\quad）\\
\quad A. 原子是实心的 \quad B. 原子内部有一个很小的原子核 \quad C. 电子在固定轨道上运动 \quad D. 原子可以再分\\
7. （填空）核反应中亏损的质量$\Delta m$对应的能量$\Delta E =$\_\_\_\_\_。核电站利用的是\_\_\_\_\_变释放的能量。\\
8. （判断）太阳内部发生的是核聚变反应。（\quad）
\end{practice}
...
\textbf{第一课 练一练：}
1. $F=kQ_1Q_2/r^2$，$E=F/q$\quad
2. ×（电场线是假想的）\quad
3. 平方，反\quad
4. $E=kQ/r^2=9\times10^9\times4\times10^{-6}/0.04=9\times10^5\ \text{N/C}$\quad
5. C\quad
6. 相似，大\quad
7. ×（电场强度是矢量）
...
2. $U=Ed$，下降\quad
3. 降低\quad
4. $W=qU=2\times10^{-6}\times500=1\times10^{-3}\ \text{J}$\quad
5. 伏特（V），电压\quad
6. A\quad
7. $\surd$（$U=Ed$，$E$不变时$d$越大$U$越大）

\textbf{第三课 练一练：}
1. $C=Q/U$\quad
2. ×（电容由极板本身结构决定，与带电量无关）\quad
3. 类平抛（匀变速曲线）\quad
4. $Q=CU=20\times10^{-6}\times50=1\times10^{-3}\ \text{C}$\quad
5. 极板面积，极板间距\quad
6. C\quad
7. ×（间距越大，电容越小）
...
2. 最大\quad
3. 等于，极大（$I=E/r$）\quad
4. $I=6/(5+1)=1\ \text{A}$，$U=IR=5\ \text{V}$\quad
5. A\quad
6. 高\quad
7. ×（短路时$U=0$）

\textbf{第五课 练一练：}
1. $F=BIL\sin\theta$，左手\quad
2. 0\quad
3. 特斯拉，T\quad
4. $F=BIL\sin30^\circ=0.5\times2\times0.3\times0.5=0.15\ \text{N}$\quad
5. 手掌，电流，安培力\quad
6. D\quad
7. $\surd$

\textbf{第六课 练一练：}
1. $F=qvB\sin\theta$，不做\quad
2. $r=mv/(qB)$\quad
3. ×（周期$T=2\pi m/(qB)$与速度无关）\quad
4. $r=mv/(qB)=6.64\times10^{-27}\times2\times10^6/(3.2\times10^{-19}\times1)\approx0.0415\ \text{m}=4.15\ \text{cm}$\quad
5. 匀速圆周，$2\pi m/(qB)$，无关\quad
6. C\quad
7. $\surd$

\textbf{第七课 练一练：}
1. $E=n\Delta\Phi/\Delta t$\quad
2. 增反减同\quad
3. $E=BLv$\quad
4. $E=n\Delta\Phi/\Delta t=100\times(0.07-0.02)/0.1=50\ \text{V}$\quad
5. 垂直磁场，韦伯（Wb）\quad
6. B\quad
7. $\surd$（$E=BLv$，$B$、$L$不变时$E\propto v$）

\textbf{第八课 练一练：}
1. 50，有效\quad
2. $n_1/n_2$\quad
3. 减小，减小\quad
4. $U_2=U_1\times n_2/n_1=220\times5000/1000=1100\ \text{V}$\quad
5. B\quad
6. 50，0.02，$220\sqrt{2}\approx311\ \text{V}$\quad
7. ×（变压器只能改变交流电的电压）

\textbf{第九课 练一练：}
1. $T=2\pi\sqrt{L/g}$，无关\quad
2. $v=\lambda f$（或$\lambda/T$）\quad
3. ×（质点只在平衡位置附近振动，不随波迁移）\quad
4. $\lambda=v/f=1500/300=5\ \text{m}$\quad
5. $-kx$，$2\pi\sqrt{m/k}$\quad
6. C\quad
7. $\surd$
...
2. 能量，质量\quad
3. 核聚变\quad
4. ×（能否产生光电效应取决于频率是否大于截止频率，与光强无关）\quad
5. $E=h\nu=6.63\times10^{-34}\times7.5\times10^{14}\approx4.97\times10^{-19}\ \text{J}$\quad
6. B\quad
7. $\Delta mc^2$，核裂\quad
8. $\surd$
