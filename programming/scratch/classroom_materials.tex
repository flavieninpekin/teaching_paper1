...
\begin{enumerate}
  \item \textbf{[向孩子提问]} 请孩子打开 Scratch，给你看看他最近做的一个作品。问他："这个作品里你用了几种不同颜色的积木？"
  \item \textbf{[概念理解]} 用你自己的话解释：Scratch 和玩游戏有什么不同？为什么说 Scratch 是"创造工具"而不是"游戏"？
  \item \textbf{[观察]} 让孩子打开 scratch.mit.edu 的"探索"页面，找一个你看起来最有趣的作品，一起看 2 分钟，然后让孩子告诉你这个作品是怎么工作的。
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[角色扮演]} 让孩子当"导演"，你当"演员"。孩子说出一个指令序列（比如"向前走两步，然后说你好，再转个圈"），你按顺序执行。和孩子讨论：如果顺序错了会怎样？
  \item \textbf{[界面记忆]} 不看屏幕，让孩子在白板上画出 Scratch 界面的五个主要区域（舞台、角色列表、积木区、脚本区、绿旗/红点），并说出每个区域的作用。
  \item \textbf{[类比延伸]} 除了剧本和演员的类比，你还能想到其他类比来解释舞台、角色和积木的关系吗？
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[颜色分类]} 找一张 Scratch 积木截图（或打开 Scratch），让孩子指出每种颜色的积木属于哪一类，并举一个使用例子。
  \item \textbf{[重点回顾]} 请孩子演示：用黄色积木（事件）启动一个程序，用橙色积木（控制）让一个角色重复做某事，用红色积木（变量）记录一个分数。
  \item \textbf{[匹配游戏]} 你说出编程任务（如"让小猫移动 10 步后说你好"），让孩子说出需要用哪些颜色的积木。
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[概念连线]} 以下 Scratch 概念分别对应日常生活中的什么？
  \begin{center}
  \begin{tabular}{c|c}
  顺序执行 & （ ）像发微信消息 \\
  事件驱动 & （ ）像带标签的盒子 \\
  ...
  \end{tabular}
  \end{center}
  \item \textbf{[编程侦探]} 请孩子打开一个作品，找一个使用了"如果……那么"积木的地方。问孩子："如果这个条件不满足，会发生什么？" 让孩子修改条件看看效果变化。
  \item \textbf{[概念迁移]} 让孩子说说以下日常场景分别对应哪个编程概念：① 每天早上闹钟响了起床 ② 走路时左右脚交替 ③ 存钱罐里的零花钱越来越多。
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[项目分类]} 请孩子说出他做过的 Scratch 项目分别属于哪一类（游戏、动画、工具、模拟器等），并挑一个最喜欢的详细讲解给你听。
  \item \textbf{[功能分析]} 让孩子选一个别人做的 Scratch 作品，分析它用了哪些"核心概念"（循环、判断、变量、消息广播等）来工作。
  \item \textbf{[创意拓展]} 问孩子："如果你可以用 Scratch 做任何东西，你最想做什么？" 鼓励他描述具体功能，帮他分解成小步骤。
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[自我评估]} 参照 L1--L5 的能力层次表，和孩子一起讨论他目前处于哪个阶段。举一个具体的例子说明为什么。
  \item \textbf{[作品点评]} 让孩子展示一个最近的作品，你对照 L1--L5 的表格，指出他展示了哪些能力层次的表现。
  \item \textbf{[进步记录]} 和孩子一起做一个"编程成长记录表"：每个月底回顾一次，看他从 L 几进步到了 L 几，记录下这个月学到的新东西。
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[模拟对话]} 让孩子假装给你展示一个新作品，你用本课推荐的问题和他对话。练习结束后，问孩子："我刚才问的问题里，哪个最让你愿意继续说下去？"
  \item \textbf{[问题设计]} 针对孩子的某个 Scratch 作品，你自己设计 3 个"好问题"（不要问"这有什么用"这类问题），然后实际问一遍，观察孩子的反应。
  \item \textbf{[沟通反思]} 回想最近一周你和孩子关于编程的对话：你有没有无意中说了"应该避免的问题"里的话？如果有，下次可以换成什么说法？
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[展示计划]} 和孩子一起制定一个"作品展示日历"，每两周一次，固定在周日晚上。第一次展示时，你帮忙计时，看看孩子能否在 5 分钟内讲清楚作品的核心功能。
  \item \textbf{[作品收集]} 帮孩子把他的 Scratch 作品（.sb3 文件）整理到一个文件夹里，按日期命名。一年后回看，就是最好的成长记录。
  \item \textbf{[观众扩展]} 鼓励孩子把作品展示给爷爷奶奶或其他亲戚看。面对不同的观众，孩子需要调整讲解方式——这是一种很重要的能力。
\end{enumerate}
...
\begin{enumerate}
  \item \textbf{[概念对照]} 拿出这份指南第九节的 Scratch/Python 对照表，让孩子指出一个 Scratch 积木对应的 Python 写法。比如问："Scratch 的'重复 10 次'在 Python 里怎么写？"
  \item \textbf{[过渡准备]} 和孩子聊聊：你觉得从 Scratch 到 Python，最大的挑战会是什么？你准备好了吗？让孩子说出他的担心和期待。
  \item \textbf{[鼓励信]} 给孩子写一张小卡片，写上你为他感到骄傲的一件事（比如"你今天调试那个 bug 很有耐心"）。这比任何物质奖励都更有力量。
\end{enumerate}
...
\section*{\textcolor{blue!70!black}{亲子练习参考答案}}

\subsection*{一、Scratch 到底是什么？}
\begin{enumerate}
  \item 无标准答案——鼓励孩子展示并讲解。重点观察他是否能清楚说出每种积木的作用。
  \item Scratch 是创造工具，不是游戏。它让孩子通过拖拽积木来编程，创建自己的动画、游戏和互动故事。类似于"用乐高搭东西"而不是"玩拼好的乐高模型"。
  \item 无标准答案——目的是让孩子了解 Scratch 社区的多样性，拓宽视野。
\end{enumerate}

\subsection*{二、Scratch 界面长什么样？}
\begin{enumerate}
  \item 通过角色扮演，孩子体会到"顺序"的重要性——指令的顺序决定结果。如果顺序错了，结果可能完全不同。
  \item 五个区域：舞台（程序运行的地方）、角色列表（显示有哪些角色）、积木区（所有可用的积木）、脚本区（拖积木拼成程序）、绿旗/红点（开始/停止）。
  \item 无标准答案——鼓励创造性类比，如"角色像棋子，舞台像棋盘，积木像走棋规则"。
\end{enumerate}

\subsection*{三、积木块有哪些颜色？各代表什么？}
\begin{enumerate}
  \item 无标准答案——让孩子展示并分类，验证他对积木功能的理解是否正确。
  \item 黄色（事件）积木启动程序（如"当绿旗被点击"），橙色（控制）积木实现循环或判断（如"重复执行""如果……那么"），红色（变量）积木记录数据（如"将得分增加1"）。
  \item 移动10步→运动（深蓝），说"你好"→外观（紫），等待1秒→控制（橙）。
\end{enumerate}

\subsection*{四、孩子通过 Scratch 学了哪些编程概念？}
\begin{enumerate}
  \item 顺序执行→像按菜谱一步一步做；事件驱动→像门铃响了才去开门；循环→像每天刷牙重复做；条件判断→像十字路口绿灯走红灯停；变量→像带标签的盒子；消息广播→像发微信消息。
  \item 无标准答案——鼓励孩子动手尝试，理解条件判断的"分支"作用。
  \item ① 事件驱动（闹钟响了触发起床动作）；② 循环（左右脚交替重复）；③ 变量（存钱罐金额不断增加）。
\end{enumerate}

\subsection*{五、孩子能用 Scratch 做什么？}
\begin{enumerate}
  \item 无标准答案——鼓励孩子分类自己的作品，梳理自己的编程经历。
  \item 无标准答案——培养孩子分析他人作品的能力，这是 L4（调试）和 L5（抽象）的基础。
  \item 无标准答案——鼓励创意表达，帮助孩子把大想法分解成可执行的小步骤。
\end{enumerate}

\subsection*{六、如何判断孩子学得怎么样？}
\begin{enumerate}
  \item L1 模仿：照着教程做。L2 修改：改参数换角色。L3 创作：自己做项目。L4 调试：自己找 bug。L5 抽象：总结模式复用代码。
  \item 无标准答案——重点是让孩子用自己的作品来佐证自己的能力层次评估。
  \item 无标准答案——持续的记录能让孩子看到自己的进步，增强信心。
\end{enumerate}

\subsection*{七、家长可以问什么问题？}
\begin{enumerate}
  \item 无标准答案——重点是让孩子感受到你对他的作品感兴趣。开放式问题（"怎么做到的？"）比闭合式问题（"好不好玩？"）效果好。
  \item 无标准答案——好问题示例："这个角色遇到敌人时为什么会消失？""你是怎么想到要做这个的？"
  \item 无标准答案——反思的目的是建立更有效的沟通方式。"别老玩电脑"可以换成"这个做完了吗？给我展示一下？"
\end{enumerate}

\subsection*{八、怎么看到孩子的作品？}
\begin{enumerate}
  \item 无标准答案——定期展示能锻炼孩子的表达能力和自信心。5分钟是一个好的时长限制，训练孩子抓重点。
  \item 无标准答案——按时间整理作品是最好的成长档案。
  \item 无标准答案——向不同观众讲解需要调整语言和重点，这是元认知能力的体现。
\end{enumerate}

\subsection*{九、下一步：从 Scratch 到 Python}
\begin{enumerate}
  \item Scratch 的"重复 10 次"在 Python 中对应 \texttt{for i in range(10):}。Scratch 的"如果……那么"对应 Python 的 \texttt{if 条件:}。
  \item 无标准答案——倾听孩子的担心，帮助他建立信心。常见担心：记不住英文单词、打字慢、出错不知道怎么办。
  \item 无标准答案——具体的鼓励比笼统的"你真棒"更能激励孩子。
\end{enumerate}
