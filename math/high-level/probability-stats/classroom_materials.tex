...
\begin{enumerate}
  \item $\dfrac35$
  \item $0.72$（$72\%$）
  \item $P(\text{合格})=0.93$；$P(\text{甲}|\text{合格})=\dfrac{19}{31}\approx0.6129$
\end{enumerate}

\subsection*{习题3}
\begin{enumerate}
  \item $P(X=k)=\dfrac{C_5^k}{32}$（$k=0,1,\dots,5$），$E(X)=2.5$
  \item $P(X\ge8)\approx0.6778$
  \item $P(160<X<180)\approx0.6826$（$68.3\%$）
\end{enumerate}

\subsection*{习题4}
\begin{enumerate}
  \item $E(X)=1.1$，$D(X)=0.49$
  \item $E(X)=2$，$D(X)=1.6$
  \item $E(2X-1)=5$，$D(2X-1)=16$
\end{enumerate}

\subsection*{习题5}
\begin{enumerate}
  \item $\bar x=14$，$S^2=2.5$
  \item 分层抽样（按年级分层）或简单随机抽样
  \item $95\%$置信区间：$[1042.16,\,1057.84]$
  \item $\hat y=0.7+1.5x$
\end{enumerate}
