...
\begin{enumerate}
  ...
  \item $f(x)=x^3$奇函数，$f(x)=x^2+1$偶函数，$f(x)=x+\dfrac1x$奇函数
  \item $f(x)=x^2-1$
  \item $\sin75^\circ=\dfrac{\sqrt6+\sqrt2}{4}$；$\cos105^\circ=\dfrac{\sqrt2-\sqrt6}{4}$；$\sin2\alpha=\dfrac{24}{25}$
  \item 最大值$5$，最小值$-5$
\end{enumerate}

\subsection*{习题2}
\begin{enumerate}
  \item $d=2$，$a_{10}=21$，$S_{10}=120$
  \item $q=\pm3$；$q=3$时$S_5=242$，$q=-3$时$S_5=122$
  \item $\dfrac{2047}{1024}$
  \item $\dfrac9{10}$
...
\subsection*{习题3}
\begin{enumerate}
  \item 由$\dfrac{a}{b}+\dfrac{b}{a}-2=\dfrac{(a-b)^2}{ab}\ge0$得证
  \item 最小值为$4$（$x=2$时取等）
  \item $xy$的最大值为$\dfrac92$（$x=3,y=\dfrac32$时取等）
  \item 由柯西不等式$(a^2+b^2)(c^2+d^2)\ge(ac+bd)^2$得$|ac+bd|\le1$
  \item 略（数学归纳法标准步骤）
  \item 略（数学归纳法标准步骤）
\end{enumerate}
...
\subsection*{习题4}
\begin{enumerate}
  \item $(2+3i)+(1-5i)=3-2i$；$(2+3i)-(1-5i)=1+8i$
  \item $(2+3i)(1-5i)=17-7i$；$\dfrac{2+3i}{1-i}=-\dfrac12+\dfrac52i$
  \item $|z|=2$，辐角$\dfrac\pi3$，三角形式$z=2\bigl(\cos\frac\pi3+i\sin\frac\pi3\bigr)$
  \item $(1+i)^8=16$
  \item $z=-1\pm2i$
\end{enumerate}
...
\subsection*{习题5}
\begin{enumerate}
  \item $\vec a+\vec b=(3,2)$，$2\vec a-\vec b=(3,-5)$，$\vec a\cdot\vec b=-1$
  \item $x=2$
  \item $m=8$
  \item $\vec a\cdot\vec b=6$，$|\vec a+\vec b|=\sqrt{37}$
  \item $\overrightarrow{AB}\cdot\overrightarrow{AC}=6$，$\angle A=\arccos\dfrac{3}{\sqrt{34}}$
\end{enumerate}
